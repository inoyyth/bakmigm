...
class C_bookmarks extends CI_Controller {
	...
	public function Email_notification($doc_id) {
		...
		if ($role_pengguna->expired_activity === "1") {
			...
			$this->__sendEmailArchive($document, $maker, $pengguna_emails, 'pengguna');
		}
		...
		if ($role_pendistribusi->expired_activity === "1") {
			...
			$this->__sendEmailArchive($document, $maker, $pendistribusi_emails, 'pendistribusi');
		}
		...
		if ($role_atasan->expired_activity === "1") {
			...
			$this->__sendEmailArchive($document, $maker, $atasan_emails, 'atasan');
		}
		...
		if ($role_pencipta->expired_activity === "1") {
			...
			$this->__sendEmailArchive($document, $maker, $pencipta_emails, 'pencipta');
		}
	}

	private function __sendEmailArchive($document, $maker, $users, $role) {
