    ...
    function sendReminder() {
        ...
        $time_before = explode("|", $rule->time_before);
        $time_after = explode("|", $rule->time_after);
        if ($time_before && $time_after) {
            $date_before = [];
            foreach ($time_before as $before) {
                $date_before[] = date('Y-m-d', strtotime("+".$before." days"));
            }
            $date_after = [];
            foreach ($time_after as $after) {
                $date_after[] = date('Y-m-d', strtotime("-".$after." days"));
            }

            $date_reminder = array_merge($date_before, $date_after);
        
            if ($rule->rule_column === "DIPUBLIKASI") { 
                $documents = $this->db->select("*")
                            ->from('tb_document')
                            ->where_in('DOC_DATE', $date_reminder) 
                            ->where("(DOC_STATUS='DIPUBLIKASI' OR DOC_STATUS='KADALUARSA')")
                            ->result();
            } else {
                $documents = $this->db->select("*")
                            ->from('tb_document')
                            ->where_in('DOC_TGL_EXPIRED', $date_reminder) 
                            ->where("(DOC_STATUS='DIPUBLIKASI' OR DOC_STATUS='KADALUARSA')")
                            ->get()
                            ->result();
            }
            if (count($documents) > 0) {
                foreach($documents as $document) {
                    ...
                    $get_all_user_by_department = $this->db->select('a.NIP,a.FULL_NAME,a.EMAIL')
                                                ...
                                                ->where("(b.ROLES='PENCIPTA' OR b.ROLES_2='PENCIPTA' OR b.ROLES_3='PENCIPTA' OR b.ROLES_4='PENCIPTA' OR b.ROLES_5='PENCIPTA')")
                                                ->get()
                                                ->result();
