					...
					<div class="row">
						<div class="col-md-12">
							...
							<?php elseif(
								...
								$ext=='bmp' ||
								$ext=='BMP'): ?>
							<img src="<?= $url; ?>" alt="" class="img-responsive container">
							<?php else: ?>
						...
						<?php
						// get dokumen & join
						$get = $this->db->select('*')->from('tb_document')->join('tb_document_detail', 'tb_document.DOC_ID = tb_document_detail.DOC_ID', 'left')->where('tb_document.DOC_ID', $id)->like('tb_document.DOC_PENGGUNA', $SESSION_DEPARTEMENT_ID, 'BOTH')->get()->result();
						$get_doc = $this->db->get_where('tb_document_detail', array('DOC_ID' => $id))->result();
						if ($ket == 'Dokumen Utama') {
							if ($get_doc[0]->DOCD_UTAMA_STATUS == 1) {
								$url_download = base_url('C_notification/download_pdf/'.$get_doc[0]->DOCD_UTAMA.'.pdf');
							}else{
								$url_download = base_url('C_notification/download_ori/'.$get_doc[0]->DOCD_UTAMA.'.'.$get_doc[0]->DOCD_UTAMA_EXT);
							}
						}elseif($ket == 'Dokumen Pelengkap 1'){
							if ($get_doc[0]->DOCD_PELENGKAP_1_STATUS == 1) {
								$url_download = base_url('C_notification/download_pdf/'.$get_doc[0]->DOCD_PELENGKAP_1.'.pdf');
							}else{
								$url_download = base_url('C_notification/download_ori/'.$get_doc[0]->DOCD_PELENGKAP_1.'.'.$get_doc[0]->DOCD_PELENGKAP_1_EXT);
							}
						}else{
							if ($get_doc[0]->DOCD_PELENGKAP_2_STATUS == 1) {
								$url_download = base_url('C_notification/download_pdf/'.$get_doc[0]->DOCD_PELENGKAP_2.'.pdf');
							}else{
								$url_download = base_url('C_notification/download_ori/'.$get_doc[0]->DOCD_PELENGKAP_2.'.'.$get_doc[0]->DOCD_PELENGKAP_2_EXT);
							}
						}
						if(!empty($get)):
							// get joblevel for download
							$get_tipe = $this->db
							->select('*')
							->from('tb_document_structure_tipe')
							->where('DTSETE_ID', $get[0]->DOC_TIPE)
							->like('JBLL_DOWNLOAD', $SESSION_JOB_LEVEL_ID)
							->get()->result();
						// Jika dokumen dipublikasi
						if($get[0]->DOC_STATUS == 'DIPUBLIKASI'):
						?>
							<?php if(!empty($get_tipe)): ?>
							<div class="col-md-3" style="margin-top:10px;">
								<a href="<?php echo $url_download; ?>" class="btn btn-info btn-sm">Download</a>
							</div>
							<?php endif; ?>
						<?php endif; ?>
						<?php endif; ?>
