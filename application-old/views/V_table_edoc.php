...
<body class="no-skin">
				...
				<div class="page-content">
					...
					<div class="row">
						<div class="alert alert-info" style="margin-bottom:0px;">
							Tabel EDOC <button data-toggle="modal" data-target="#modal-tambah" class="btn btn-success btn-sm" style="float:right;margin-top:-5px;">Tambah</button>
						</div>
						<table id="dynamic-table" class="table table-striped table-bordered table-hover" style="cursor:pointer">
							...
							<tbody>
								...
								$is_continue = true;
								$no = 1;
								$get_data = $this->M_setting_user->DB_DATA_USER();
								...
								?>
								<tr data-toggle="modal" data-target="#modal-edit<?= $data_row->NIP; ?>">
									<td><?= $no ?></td>
									<td><?php echo $data_row->NIP; ?></td>
									<td><?php echo $data_row->UR_ID; ?></td>
									<td><?php echo $data_row->UR_PASSWORD; ?></td>
									<td><?php echo $data_row->UR_EMAIL; ?></td>
									<td><?php echo $data_row->DN_NAME; ?></td>
									<td><?php echo $data_row->RS_ID; ?></td>
									<td><?php echo $data_row->JBLL_NAME; ?></td>
									<td><?php echo $data_row->UR_GROUP; ?></td>
									<td><?php echo date('d/m/Y H:i', strtotime($data_row->UR_DATE))." WIB"; ?></td>
								</tr>
								<?php
								 $no++;
									}
	...
	<div id="modal-tambah" class="modal" tabindex="-1">
		<div class="modal-dialog modal-md">
			<div class="modal-content">
				...
				<form method="POST" action="<?= base_url('C_setting_user/insert_user'); ?>" class="form-horizontal">
					...
					<div class="row">
						<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
							...
							<div class="form-group">
								...
								<?php
									$GET_ROLE = $this->M_admin_setting->GET_ROLE();
								?>
								<select name="role" id="role" class="form-control" required>
									<option value="">Pilih</option>
									<?php foreach ($GET_ROLE as $role): ?>
									<option><?= $role->RS_ID; ?></option>
									<?php endforeach; ?>
								</select>
								<br/>
								* Setiap Roles (Pencipta, Pendistribusi ataupun Atasan Pencipta) sudah pasti Pengguna.
							</div>
							<div class="form-group">
								<label for="job_level">Job Level</label>
								<?php
									$GET_JOB_LEVEL = $this->M_setting_user->GET_JOB_LEVEL();
								?>
								<select name="job_level" id="job_level" class="form-control" required>
									<option value="">Pilih</option>
									<?php foreach ($GET_JOB_LEVEL as $joblevel): ?>
									<option value="<?= $joblevel->JBLL_ID; ?>"><?= $joblevel->JBLL_NAME; ?></option>
									<?php endforeach; ?>
								</select>
							</div>
							<div class="form-group">
	...
	<!-- MODAL VIEW EDIT -->
	<?php
		$view = $this->M_setting_user->DB_DATA_USER();
		foreach ($view as $view):
	?>
	<div id="modal-edit<?= $view->NIP; ?>" class="modal" tabindex="-1">
		<div class="modal-dialog modal-md">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal">&times;</button>
				...
				<form method="POST" action="<?= base_url('C_setting_user/update_tabel_user') ?>" class="form-horizontal">
				<div class="modal-body" style="margin: 0 15px">
					<div class="row">
						<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
							<div class="form-group">
								<label for="id">NIP</label>
								<input type="text" name="nip" id="nip" class="form-control" value="<?= $view->NIP ?>">
							</div>
							<div class="form-group">
								<label for="username">Username</label>
								<input type="text" name="username" id="username" class="form-control" value="<?= $view->UR_ID ?>">
							</div>
							<div class="form-group">
								<label for="password">Password</label>
								<input type="text" name="password" id="password" class="form-control" value="<?= $view->UR_PASSWORD ?>">
							</div>
							<div class="form-group">
								<label for="email">Email</label>
								<input type="text" name="email" id="email" class="form-control" value="<?= $view->UR_EMAIL ?>">
							</div>
							<div class="form-group">
								<label for="departemen">Departemen</label>
								<?php
								...
								<select name="departemen" id="departemen" class="form-control" required>
									<option value="">Pilih</option>
									<option value="<?= $view->DN_ID; ?>" selected><?= $view->DN_NAME; ?></option>
									<?php foreach ($GET_DEPARTEMEN as $departemen): ?>
									<option value="<?= $departemen->DN_ID; ?>"><?= $departemen->DN_NAME; ?></option>
									<?php endforeach; ?>
								</select>
							...
							<div class="form-group">
								...
								<select name="role" id="role" class="form-control" required>
									<option value="">Pilih</option>
									<option value="<?= $view->RS_ID; ?>" selected><?= $view->RS_ID; ?></option>
									<?php foreach ($GET_ROLE as $role): ?>
									<option><?= $role->RS_ID; ?></option>
									<?php endforeach; ?>
								</select>
							...
							<div class="form-group">
								...
								<select name="job_level" id="job_level" class="form-control" required>
									<option value="">Pilih</option>
									<option value="<?= $view->JBLL_ID; ?>" selected><?= $view->JBLL_NAME; ?></option>
									<?php foreach ($GET_JOB_LEVEL as $joblevel): ?>
									<option value="<?= $joblevel->JBLL_ID; ?>"><?= $joblevel->JBLL_NAME; ?></option>
									<?php endforeach; ?>
								</select>
							...
							<div class="form-group">
								<label for="user_group">User Group</label>
								<input type="text" name="user_group" id="user_group" class="form-control" value="<?= $view->UR_GROUP; ?>">
							</div>
						</div><!-- col -->
					</div><!-- row -->
				</div><!-- body -->
				<div class="modal-footer">
					<a href="<?php echo base_url('C_setting_user/delete_user/'.$view->NIP) ?>" class="btn btn-danger btn-sm">DELETE</a>
					<button type="submit" class="btn btn-success btn-sm">Tambah</button>
				</div><!-- footer -->
				</form>
			</div>
					...
					$.ajax({
						...
						success: function(response){
							...
							$.each(response,function(index,data){
								...
								$('#job_level').val(data['JBLL_NAME']);
								$('#departemen').val(data['DN_NAME']);
							});
						}
					});
				}
