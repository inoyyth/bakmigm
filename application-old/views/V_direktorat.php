...
<body class="no-skin">
					...
					<div class="row">
						...
						<table id="dynamic-table" class="table table-striped table-bordered table-hover" style="cursor:pointer">
							...
							<tbody>
								...
								$is_continue = true;
								$no = 1;
								$get_data = $this->M_admin_setting->GET_DIREKTORAT_ALL();
								...
								?>
								<tr data-toggle="modal" data-target="#modal-edit<?= $data_row->SODT_ID; ?>">
									<td><?= $no ?></td>
									<td><?php echo $data_row->DT_ID; ?></td>
									<td><?php echo $data_row->DT_NAME; ?></td>
									<td><?php echo $data_row->SODT_LVL; ?></td>
									<td><?php echo date('d/m/Y H:i', strtotime($data_row->SODT_TIME))." WIB"; ?></td>
	...
	<!-- MODAL VIEW EDIT -->
	<?php
		$view = $this->M_admin_setting->GET_DIREKTORAT_ALL();
		foreach ($view as $view):
	?>
	<div id="modal-edit<?= $view->SODT_ID; ?>" class="modal" tabindex="-1">
		<div class="modal-dialog modal-md">
			<div class="modal-content">
				...
				<form method="POST" action="<?= base_url('C_setting_structure_organization/update_direktorat') ?>" class="form-horizontal">
				<div class="modal-body" style="margin: 0 15px">
					<div class="row">
						<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
							<input type="hidden" name="id_sod" value="<?= $view->SODT_ID; ?>">
							<div class="form-group">
								<label for="id">ID Direktorat :</label>
								<input type="text" name="id" id="id" class="form-control" value="<?= $view->DT_ID; ?>">
							</div>
							<div class="form-group">
								<label for="nama">Nama Direktorat :</label>
								<input type="text" name="nama" id="nama" class="form-control" value="<?= $view->DT_NAME; ?>" required>
							</div>
							<div class="form-group">
								<label for="level">Level :</label>
								<input type="text" name="level" id="level" class="form-control" value="01 - BOD" required>
							</div>
						</div><!-- col -->
					</div><!-- row -->
				</div><!-- body -->
