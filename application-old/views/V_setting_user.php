...
<body class="no-skin">
						...
						<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
							<div class="panel panel-warning">
								...
								<div class="list-group">
									<a href="<?= base_url('C_setting_user/table_hris');?>" class="list-group-item list-group-item-action">Table HRIS</a>
									<a href="<?= base_url('C_setting_user/table_edoc');?>" class="list-group-item list-group-item-action">Table EDOC</a>
								</div>
							</div>
						</div>
					</div>
	...
	<!-- MODAL TAMBAH -->
	<div id="modal-tambah" class="modal" tabindex="-1"></div>
	<!-- END MODAL TAMBAH -->
	<!------------------------------------------------------------------------------------------------->
	...
	<script type="text/javascript">
		jQuery(function($) {
			$('#user').change(function(){
				var id_key = document.getElementById('user');
				id_key = id_key.options[id_key.selectedIndex].value;
				
				//AJAX Request
				$.ajax({
					url: '<?=base_url('C_setting_user/get_user')?>',
					type: 'POST', 
					data: {id_key: id_key},
					success: function(response){
						...
						$.each(response,function(index,data){
							$('#email').val(data['UR_EMAIL']);
							$('#role').val(data['RS_ID']);
							if (data['RS_ID'] == "ATASAN PENCIPTA") {
								$('#level').val('02 - Divisi');
							}else{
								$('#level').val('03 - Departemen');
							}
							$('#parent').val(data['DI_NAME']);
							$('#departemen').val(data['DN_CODE']);
							$('#jb_level').val(data['JBLL_ID']);
